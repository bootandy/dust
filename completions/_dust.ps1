...
Register-ArgumentCompleter -Native -CommandName 'dust' -ScriptBlock {
    ...
    $ArrayFormat           = @(
                     [pscustomobject]@{MyParameter='si';MyDescription='SI prefix (powers of 1000)'}
                     [pscustomobject]@{MyParameter='b' ;MyDescription='byte (B)'}
                     [pscustomobject]@{MyParameter='k' ;MyDescription='kibibyte (KiB)'}
                     [pscustomobject]@{MyParameter='m' ;MyDescription='mebibyte (MiB)'}
                     [pscustomobject]@{MyParameter='g' ;MyDescription='gibibyte (GiB)'}
                     [pscustomobject]@{MyParameter='t' ;MyDescription='tebibyte (TiB)'}
                     [pscustomobject]@{MyParameter='kb';MyDescription='kilobyte (kB)'}
                     [pscustomobject]@{MyParameter='mb';MyDescription='megabyte (MB)'}
                     [pscustomobject]@{MyParameter='gb';MyDescription='gigabyte (GB)'}
                     [pscustomobject]@{MyParameter='tb';MyDescription='terabyte (TB)'}
    )
    ...
    $completions = @(switch  -Wildcard ($command) {
        ...
        '*;--output-format' {
            $ArrayFormat |
            ForEach-Object {[System.Management.Automation.CompletionResult]::new($_.MyParameter, $_.MyParameter, [CompletionResultType]::ParameterName, $_.MyDescription)}
            break
        }    
        default {
           #[CompletionResult]::new('-d'                  , '-d'                  , [CompletionResultType]::ParameterName, 'Depth to show')
            [CompletionResult]::new('--depth'             , '--depth'             , [CompletionResultType]::ParameterName, 'Depth to show <DEPTH>')
           #[CompletionResult]::new('-T'                  , '-T '                 , [CompletionResultType]::ParameterName, 'Number of threads to use')
            [CompletionResult]::new('--threads'           , '--threads'           , [CompletionResultType]::ParameterName, 'Number of threads to use <THREADS>')
            [CompletionResult]::new('--config'            , '--config'            , [CompletionResultType]::ParameterName, 'Specify a config file to use <FILE>')
           #[CompletionResult]::new('-n'                  , '-n'                  , [CompletionResultType]::ParameterName, 'Number of lines of output to show. (Default is terminal_height - 10)')
            [CompletionResult]::new('--number-of-lines'   , '--number-of-lines'   , [CompletionResultType]::ParameterName, 'Number of lines of output to show. <NUMBER> (Default is terminal_height - 10)')
           #[CompletionResult]::new('-X'                  , '-X '                 , [CompletionResultType]::ParameterName, 'Exclude any file or directory with this path')
            [CompletionResult]::new('--ignore-directory'  , '--ignore-directory'  , [CompletionResultType]::ParameterName, 'Exclude any file or directory with this path <PATH>')
           #[CompletionResult]::new('-I'                  , '-I '                 , [CompletionResultType]::ParameterName, 'Exclude any file or directory with a regex matching that listed in this file, the file entries will be added to the ignore regexs provided by --invert_filter')
            [CompletionResult]::new('--ignore-all-in-file', '--ignore-all-in-file', [CompletionResultType]::ParameterName, 'Exclude any file or directory with a regex matching that listed in this file, the file entries will be added to the ignore regexs provided by <FILE>--invert_filter')
           #[CompletionResult]::new('-z'                  , '-z'                  , [CompletionResultType]::ParameterName, 'Minimum size file to include in output')
            [CompletionResult]::new('--min-size'          , '--min-size'          , [CompletionResultType]::ParameterName, 'Minimum size file to include in output <MIN_SIZE>')
           #[CompletionResult]::new('-v'                  , '-v'                  , [CompletionResultType]::ParameterName, 'Exclude filepaths matching this regex. To ignore png files type: -v "\.png$"')
            [CompletionResult]::new('--invert-filter'     , '--invert-filter'     , [CompletionResultType]::ParameterName, 'Exclude filepaths matching this regex. To ignore png files type: <REGEX> --invert-filter "\.png$"')
           #[CompletionResult]::new('-e'                  , '-e'                  , [CompletionResultType]::ParameterName, 'Only include filepaths matching this regex. For png files type: -e "\.png$"')
            [CompletionResult]::new('--filter'            , '--filter'            , [CompletionResultType]::ParameterName, 'Only include filepaths matching this regex. For png files type: <REGEX> --filter "\.png$"')
           #[CompletionResult]::new('-w'                  , '-w'                  , [CompletionResultType]::ParameterName, 'Specify width of output overriding the auto detection of terminal width')
            [CompletionResult]::new('--terminal-width'    , '--terminal-width'    , [CompletionResultType]::ParameterName, 'Specify width of output overriding the auto detection of terminal width <WIDTH>')
           #[CompletionResult]::new('-o'                  , '-o'                  , [CompletionResultType]::ParameterName, 'Changes output display size. si will print sizes in powers of 1000. b k m g t kb mb gb tb will print the whole tree in that size')
            [CompletionResult]::new('--output-format'     , '--output-format'     , [CompletionResultType]::ParameterName, 'Changes output display size. <FORMAT> si will print sizes in powers of 1000. b k m g t kb mb gb tb will print the whole tree in that size')
           #[CompletionResult]::new('-S'                  , '-S '                 , [CompletionResultType]::ParameterName, 'Specify memory to use as stack size - use if you see: ''fatal runtime error: stack overflow'' (default low memory=1048576, high memory=1073741824)')
            [CompletionResult]::new('--stack-size'        , '--stack-size'        , [CompletionResultType]::ParameterName, 'Specify memory to use as stack size <STACK_SIZE> - use if you see: ''fatal runtime error: stack overflow'' (default low memory=1048576, high memory=1073741824)')
           #[CompletionResult]::new('-M'                  , '-M '                 , [CompletionResultType]::ParameterName, '+/-n matches files modified more/less than n days ago, and n matches files modified exactly n days ago, days are rounded down.That is +n => (−∞, curr−(n+1)), n => [curr−(n+1), curr−n), and -n => (𝑐𝑢𝑟𝑟−𝑛, +∞)')
            [CompletionResult]::new('--mtime'             , '--mtime'             , [CompletionResultType]::ParameterName, '<MTIME> +/-n matches files modified more/less than n days ago, and n matches files modified exactly n days ago, days are rounded down.That is +n => (−∞, curr−(n+1)), n => [curr−(n+1), curr−n), and -n => (𝑐𝑢𝑟𝑟−𝑛, +∞)')
           #[CompletionResult]::new('-A'                  , '-A '                 , [CompletionResultType]::ParameterName, 'just like -mtime, but based on file access time')
            [CompletionResult]::new('--atime'             , '--atime'             , [CompletionResultType]::ParameterName, '<ATIME> just like -mtime, but based on file access time')
           #[CompletionResult]::new('-y'                  , '-y'                  , [CompletionResultType]::ParameterName, 'just like -mtime, but based on file change time')
            [CompletionResult]::new('--ctime'             , '--ctime'             , [CompletionResultType]::ParameterName, '<CTIME> just like -mtime, but based on file change time')
            [CompletionResult]::new('--files0-from'       , '--files0-from'       , [CompletionResultType]::ParameterName, 'run dust on NUL-terminated file names specified in file <FILES0_FROM>; if argument is -, then read names from standard input')
            [CompletionResult]::new('--collapse'          , '--collapse'          , [CompletionResultType]::ParameterName, 'Keep these directories collapsed <COLLAPSE>')
           #[CompletionResult]::new('-m'                  , '-m'                  , [CompletionResultType]::ParameterName, 'Directory ''size'' is max filetime of child files instead of disk size. while a/c/m for last accessed/changed/modified time')
            [CompletionResult]::new('--filetime'          , '--filetime'          , [CompletionResultType]::ParameterName, 'Directory ''size'' is max filetime of child files instead of disk size. while a/c/m for last accessed/changed/modified time <FILETIME>')
