...
fn display_node(
    ...
    let bar_text = generate_bar(parent_bar, display_data, percent_size, longest_string_length);
...
fn generate_bar(
    ...
    new_bar
    // let c = BLOCKS[0]; //"▇"
    // let markers = (repeat(" ").take(num_spaces).collect::<String>())
    //     + (&(repeat(c).take(num_bars).collect::<String>()));
    // markers
}
